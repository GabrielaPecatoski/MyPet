import {
  type QuestionStatus,
  UserQuestion,
} from "@faq/questions/domain/models/user-question.entity";
...
export class DrizzleQuestionRepository implements QuestionRepository {
  ...
  async create(q: UserQuestion): Promise<UserQuestion> {
    const [row] = await this.drizzleService.db
      .insert(userQuestionsSchema)
      .values({
        userId: q.userId,
        userName: q.userName,
        userRole: q.userRole,
  ...
  async findById(id: string): Promise<UserQuestion | null> {
    const [row] = await this.drizzleService.db
      .select()
      .from(userQuestionsSchema)
      .where(eq(userQuestionsSchema.id, id))
      .limit(1);
    return row
      ? UserQuestion.restore({
          ...row,
          status: row.status as QuestionStatus,
          answer: row.answer ?? undefined,
          answeredAt: row.answeredAt ?? undefined,
  ...
  async findByUserId(userId: string): Promise<UserQuestion[]> {
    const rows = await this.drizzleService.db
      .select()
      .from(userQuestionsSchema)
      .where(eq(userQuestionsSchema.userId, userId));
    return rows.map(
      (r) =>
        UserQuestion.restore({
          ...r,
  ...
  async findAll(status?: string): Promise<UserQuestion[]> {
    const rows = status
      ? await this.drizzleService.db
          .select()
          .from(userQuestionsSchema)
          .where(eq(userQuestionsSchema.status, status))
