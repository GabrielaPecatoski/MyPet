...
import { bootFlutter, tapButton, tapText, expectText, waitForText, byText, skipOnboardingIfPresent } from './_helpers';
...
test('selecionar Tutor muda botão para "Continuar como Tutor"', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Sou Tutor');
  await waitForText(page, /Continuar como Tutor/);
});

test('selecionar Estabelecimento muda botão para "Continuar como Estabelecimento"', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Estabelecimento');
  await waitForText(page, /Continuar como Estabelecimento/);
});

test('selecionar Veterinário muda botão para "Continuar como Veterinário"', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Sou Veterinário');
  await waitForText(page, /Continuar como Veteri/);
});

test('selecionar Motorista muda botão para "Continuar como Motorista"', async ({ page }) => {
  await goToWelcome(page);
...
test('continuar como Tutor abre tela de registro com header Tutor', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Sou Tutor');
...
test('continuar como Veterinário abre registro com campos de CRMV', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Sou Veterinário');
...
test('continuar como Motorista abre registro com campos de veículo', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Sou Motorista');
...
test('continuar como Estabelecimento abre registro com campo Nome do estabelecimento', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Estabelecimento');
...
test('"Entrar agora" no register leva de volta ao login (não para welcome)', async ({ page }) => {
  await goToWelcome(page);
  await tapText(page, 'Sou Tutor');
  await tapButton(page, /Continuar como Tutor/);
  await waitForText(page, 'Nome completo');
