...
test('home exibe chips de filtro: Todos, Banho, Tosa, Veterinário, Acessórios', async ({ page }) => {
...
test('clicar em Veterinário exibe seção de veterinários disponíveis', async ({ page }) => {
  await bootAndLogin(page, cliente.email, cliente.password);
  await waitForText(page, 'Emergência Veterinária');
...
test('clicar em Veterinário e voltar em Todos restaura o carousel', async ({ page }) => {
  await bootAndLogin(page, cliente.email, cliente.password);
  await waitForText(page, 'Emergência Veterinária');
  await tapText(page, 'Veterinário');
  await waitForText(page, /Veterinários disponíveis|Nenhum veterinário|Clínicas/);
  await tapText(page, 'Todos');
  await waitForText(page, 'Mais Bem Avaliados');
});

test('chip Banho filtra estabelecimentos (não quebra a UI)', async ({ page }) => {
  await bootAndLogin(page, cliente.email, cliente.password);
  await waitForText(page, 'Emergência Veterinária');
...
test('seção Veterinários mostra badge Domiciliar se vet ativa o serviço', async ({ page }) => {
  ...
  const hasDomiciliar = await byText(page, 'Domiciliar').first().isVisible({ timeout: 5000 }).catch(() => false);
