...
test.beforeAll(async () => {
  ...
  const pet = await createPet(api, cliente, {
    name: `Thor ${Date.now().toString().slice(-4)}`,
  });
  petName = pet.name;
...
test("transporte só aparece ao motorista depois que o estabelecimento aceita", async ({
  page,
}) => {
  ...
  await expect
    .poll(async () => (await getBooking(api, cliente, bookingId)).status, {
      timeout: 20_000,
    })
    .toBe("CONFIRMADO");
