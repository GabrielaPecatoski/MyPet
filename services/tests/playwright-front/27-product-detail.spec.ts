...
test.beforeAll(async () => {
  ...
  const owner = await registerUser(api, { role: 'VENDEDOR', businessName: 'Estab Detail E2E' });
  const estab = await createEstablishment(api, owner, { name: `Pet Shop Detail ${Date.now()}` });
...
test('detalhe exibe preço do produto', async ({ page }) => {
  ...
  await expectText(page, new RegExp(produtoPreco.toFixed(2).replace('.', '[.,]')));
});
...
test('adicionar 2 unidades via detalhe mostra snackbar correto', async ({ page }) => {
  await openDetail(page);
  await tapButton(page, 'Aumentar quantidade');
  await waitForText(page, '2');
...
test('adicionar 1 unidade via detalhe mostra snackbar no singular', async ({ page }) => {
  await openDetail(page);
  await tapButton(page, 'Adicionar ao Carrinho');
  await expectText(page, '1 unidade adicionada ao carrinho');
});
test('badge "X unidades no carrinho" aparece na tela de detalhe após adicionar', async ({ page }) => {
  await openDetail(page);
  await tapButton(page, 'Adicionar ao Carrinho');
