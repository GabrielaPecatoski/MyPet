...
test.beforeAll(async () => {
  ...
  const eUnpaid = await createEstablishment(api, ownerUnpaid, {
    name: `PetUnp ${Date.now() + 2}`,
  });
...
test('botão "Saiu para entrega" avança pedido e mostra badge "Indo até o endereço"', async ({
  ...
  await pollTap(
    page,
    "Saiu para entrega",
    /Indo até o endereço|Finalizar pedido/,
  );
  await expectText(page, "Indo até o endereço");
});

test('botão "Finalizar pedido" conclui o ciclo e mostra banner "Pedido finalizado"', async ({
  page,
}) => {
  await seedPaidOrder(api, ownerDelivery, estabDeliveryId);
  await abrirPedidosEstab(page, ownerDelivery, "Saiu para entrega");
...
test('botão "Marcar como pronto" avança pickup e mostra "Pronto p/ retirada"', async ({
  ...
  await pollTap(
    page,
    "Marcar como pronto",
    /Pronto p\/ retirada|Finalizar pedido/,
  );
  await expectText(page, "Pronto p/ retirada");
});
...
test("pedido aguardando pagamento não exibe botão de avanço", async ({
  page,
}) => {
  await abrirPedidosEstab(page, ownerUnpaid, "Aguardando pagamento do cliente");
  const btn = await byText(page, /Saiu para entrega|Marcar como pronto/)
    .first()
    .isVisible({ timeout: 2_000 })
    .catch(() => false);
...
test('pedido finalizado exibe banner verde "Pedido finalizado"', async ({
  page,
}) => {
  await abrirPedidosEstab(page, ownerFinal, "Pedido finalizado");
  await expectText(page, "Pedido finalizado");
