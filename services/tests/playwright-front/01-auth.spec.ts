...
import { bootFlutter, login, tapButton, tapText, fillNth, expectText, waitForText } from './_helpers';
...
test('login com usuário válido leva à home do cliente', async ({ page }) => {
  ...
  await login(page, cliente.email, cliente.password);
...
test('login com senha errada mostra erro e permanece no login', async ({ page }) => {
  ...
  await expect.poll(async () => (await page.locator('flt-semantics[role="button"]').filter({ hasText: 'Entrar' }).count())).toBeGreaterThan(0);
});
