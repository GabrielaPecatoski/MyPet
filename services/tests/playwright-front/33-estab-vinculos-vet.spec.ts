...
test.beforeAll(async () => {
  ...
  owner = await registerUser(api, { role: 'VENDEDOR', businessName: 'Estab Vinculos E2E' });
  estab = await createEstablishment(api, owner, { name: `Clínica Vinculos E2E ${Date.now()}` });
});

test.afterAll(async () => { await api.dispose(); });

test('menu Veterinários está acessível no perfil do estab', async ({ page }) => {
...
test('tela de vets do estab exibe header e estado vazio ou lista', async ({ page }) => {
  await bootAndLogin(page, owner.email, owner.password);
  await tapText(page, 'Perfil');
...
test('cadastrar vet novo via UI vincula ao estab e aparece na lista', async ({ page }) => {
  ...
  await waitForText(page, /cadastrado e associado|Associado|Veterinários associados/i, 40_000);
});

test('buscar vet existente por CPF permite associá-lo', async ({ page }) => {
  // Cria um vet independente via API para depois buscar na UI
  const vetUser = await registerUser(api, { role: 'VETERINARIO', namePrefix: 'Vet Busca E2E' });
  ...
  await tapButton(page, 'Buscar');
  await waitForText(page, /Vet Busca E2E|CRMV|Associar a este estabelecimento|já está associado|não encontrado/, 20_000);
});
