...
import { bootAndLogin, tapText, waitForText, expectText, scrollToText } from './_helpers';
import {
  apiContext, registerUser, createEstablishment, registerDriver, SeededUser,
} from './_api';
...
test.beforeAll(async () => {
  ...
  owner = await registerUser(api, { role: 'VENDEDOR', businessName: 'Estab Motoristas Lista E2E' });
  const estab = await createEstablishment(api, owner, { name: `Pet Shop Mot Lista ${Date.now()}` });
  estabId = estab.id;
  const d1 = await registerDriver(api, owner, estabId, { name: `Motorista Um ${Date.now().toString().slice(-4)}` });
  const d2 = await registerDriver(api, owner, estabId, { name: `Motorista Dois ${(Date.now() + 1).toString().slice(-4)}` });
  driver1Name = d1.name;
  driver2Name = d2.name;
});
...
test('lista de Motoristas no perfil do estab exibe os motoristas associados', async ({ page }) => {
