...
import {
  ...
  scrollToText, byText, fill, pollTap, leafByText,
} from './_helpers';

// Polling sem waitForFunction para evitar cap do actionTimeout (Playwright 1.40+)
const sleep = (ms: number) => new Promise(r => setTimeout(r, ms));
async function pollForSlots(page: import('@playwright/test').Page, timeoutMs: number): Promise<boolean> {
...
import {
  ...
  addVariableService, setSchedule, createPet, SeededUser,
} from './_api';
...
test('cliente agenda consulta com preço fixo → dialog de pagamento', async ({ page }) => {
  await bootAndLogin(page, cliente.email, cliente.password);
  await waitForText(page, 'Emergência Veterinária');

  // Navegar para a clínica pelo filtro Veterinário
  await leafByText(page, 'Veterinário').first().click({ force: true });
  await waitForText(page, /Clínicas e Pet shops|Veterinários disponíveis/, 60_000);
  await scrollToText(page, new RegExp('Clínica Vet E2E'), 40);
  await leafByText(page, /Clínica Vet E2E/).first().click({ force: true });
  ...
  await page.waitForFunction(
    () => Array.from(document.querySelectorAll('flt-semantics'))
      .some(el => (el.textContent ?? '').includes('Consulta E2E')),
    { timeout: 30_000 },
  );
  ...
  await sleep(500);
  await page.locator('flt-semantics[role="button"]').filter({ hasText: 'Hoje' })
  ...
  await page.locator('flt-semantics[role="button"]').filter({ hasText: /^\d{2}:\d{2}$/ })
    .first().click({ force: true });
...
test('cliente agenda consulta preço variável → dialog sem pagamento', async ({ page }) => {
  await bootAndLogin(page, cliente.email, cliente.password);
  await waitForText(page, 'Emergência Veterinária');
  ...
  await page.waitForFunction(
    () => Array.from(document.querySelectorAll('flt-semantics'))
      .some(el => (el.textContent ?? '').includes('Consulta Sob Consulta')),
    { timeout: 30_000 },
  );
  await scrollToText(page, /Consulta Sob Consulta/, 20);
  await leafByText(page, /Consulta Sob Consulta/).first().click({ force: true });
