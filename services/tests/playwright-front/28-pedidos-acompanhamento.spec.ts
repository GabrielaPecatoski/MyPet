...
test.beforeAll(async () => {
  ...
  owner = await registerUser(api, {
    role: "VENDEDOR",
    businessName: "Estab Track E2E",
  });
  const estab = await createEstablishment(api, owner, {
    name: `Pet Shop Track ${Date.now()}`,
  });
  estabId = estab.id;
  ...
  await payOrder(api, clienteEmAndamento, orderAndamento.id, {
    ...
    deliveryAddress: "Rua dos Testes, 200",
  });
  ...
  await payOrder(api, clienteFinalizado, orderFinalizado.id, {
    method: "PIX",
    deliveryMethod: "DELIVERY",
    deliveryAddress: "Rua dos Testes, 300",
  });
...
async function openPedidosTab(
  page: Parameters<typeof bootAndLogin>[0],
  email: string,
  senha: string,
) {
  ...
  await pollTap(
    page,
    "Pedidos",
    /Enviando já|Indo até|Finalizado|Nenhum pedido/,
  );
}
test("aba Pedidos mostra pedido ENVIANDO com label correto", async ({
  page,
}) => {
  await openPedidosTab(
    page,
    clienteEmAndamento.email,
    clienteEmAndamento.password,
  );
...
test("pedido ENVIANDO mostra barra de progresso com etapas", async ({
  page,
}) => {
  await openPedidosTab(
    page,
...
test('filtro "Em andamento" mantém pedido ENVIANDO visível', async ({
  page,
}) => {
  await openPedidosTab(
    page,
...
test('filtro "Finalizados" oculta pedido ENVIANDO e mostra mensagem vazia', async ({
  page,
}) => {
  await openPedidosTab(
    page,
  ...
  const visivel = await byText(page, "Enviando já")
    .first()
...
test('filtro "Finalizados" mostra pedido FINALIZADO', async ({ page }) => {
  await openPedidosTab(
    page,
    clienteFinalizado.email,
    clienteFinalizado.password,
  );
  await byText(page, "Finalizados").first().click({ force: true });
  await waitForText(page, "Finalizado");
});
test('filtro "Em andamento" oculta pedido FINALIZADO e mostra mensagem vazia', async ({
  page,
}) => {
  await openPedidosTab(
    page,
  ...
  const visivel = await byText(page, produtoNomeFinalizado)
    .first()
    .isVisible({ timeout: 2_000 })
    .catch(() => false);
  expect(visivel).toBe(false);
...
test("pedido FINALIZADO mostra barra de progresso completa", async ({
  page,
}) => {
  await openPedidosTab(
    page,
...
test("pedido ENVIANDO não exibe label de cancelado", async ({ page }) => {
  ...
  const cancelado = await byText(page, "Pedido cancelado")
    .first()
    .isVisible({ timeout: 2_000 })
    .catch(() => false);
