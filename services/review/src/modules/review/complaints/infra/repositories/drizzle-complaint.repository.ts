...
export class DrizzleComplaintRepository implements ComplaintRepository {
  ...
  async delete(id: string): Promise<void> {
    await this.drizzleService.db
      .delete(complaintsSchema)
      .where(eq(complaintsSchema.id, id));
  }
  ...
  async findAll(status?: string): Promise<Complaint[]> {
    const rows = status
      ? await this.drizzleService.db
          .select()
          .from(complaintsSchema)
          .where(eq(complaintsSchema.status, status))
  ...
  async findByEstablishmentId(establishmentId: string): Promise<Complaint[]> {
    const rows = await this.drizzleService.db
      .select()
      .from(complaintsSchema)
      .where(eq(complaintsSchema.establishmentId, establishmentId));
    return rows.map(
      (r) =>
        Complaint.restore({
          ...r,
