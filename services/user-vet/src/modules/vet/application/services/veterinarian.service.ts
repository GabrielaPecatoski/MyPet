import { CreateVeterinarianDto, UpdateVetAvailabilityDto } from '@vet/vet/application/dto/create-veterinarian.dto';
...
import { VETERINARIAN_REPOSITORY, type VeterinarianRepository } from '@vet/vet/domain/repositories/veterinarian-repository.interface';
...
export class VeterinarianService {
  ...
  async findPending(): Promise<VeterinarianDto[]> {
    return (await this.repo.findByStatus('PENDENTE')).map((v) => VeterinarianDto.fromVet(v)!);
  }

  async findAvailable(): Promise<VeterinarianDto[]> {
    return (await this.repo.findAvailable()).map((v) => VeterinarianDto.fromVet(v)!);
  }

  async findByEstablishment(establishmentId: string): Promise<VeterinarianDto[]> {
    return (await this.repo.findByEstablishment(establishmentId)).map((v) => VeterinarianDto.fromVet(v)!);
  }

  async findUnassociated(): Promise<VeterinarianDto[]> {
    return (await this.repo.findUnassociated()).map((v) => VeterinarianDto.fromVet(v)!);
  }
  ...
  async updateAvailability(id: string, dto: UpdateVetAvailabilityDto): Promise<VeterinarianDto> {
    const vet = await this.repo.findById(id);
    if (!vet) throw new NotFoundException('Veterinário não encontrado');
    if (dto.disponivel === true && vet.status !== 'ATIVO') {
      throw new ForbiddenException('Veterinário ainda não foi aprovado pelo administrador');
    }
    if (dto.disponivel !== undefined) vet.withDisponivel(dto.disponivel);
    if (dto.atendeDomicilio !== undefined) vet.withAtendeDomicilio(dto.atendeDomicilio);
  ...
  async associate(id: string, establishmentId: string): Promise<VeterinarianDto> {
    const vet = await this.repo.findById(id);
    if (!vet) throw new NotFoundException('Veterinário não encontrado');
    if (vet.establishmentId) throw new ConflictException('Veterinário já está associado a um estabelecimento');
  ...
  async createEmergencyCall(vetId: string, dto: CreateEmergencyCallDto) {
    ...
    try {
      await this.messagingService.assertExchange(EmergencyExchangeName.VET_CALL);
      await this.messagingService.publish(EmergencyExchangeName.VET_CALL, EmergencyRoutingKey.VET_CALL, {
        callId: call.id,
        vetId,
        vetName: vet.name,
        callerName: dto.callerName,
