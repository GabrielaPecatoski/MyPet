...
export class DrizzleOrderRepository implements OrderRepository {
  ...
  async updateStatus(id: string, status: OrderStatus): Promise<void> {
    await this.drizzleService.db.update(ordersSchema).set({ status }).where(eq(ordersSchema.id, id));
  }

  async updateDelivery(id: string, deliveryMethod: DeliveryMethod, deliveryAddress?: string): Promise<void> {
  ...
  private async withItems(orders: (typeof ordersSchema.$inferSelect)[]): Promise<Order[]> {
    const result: Order[] = [];
