 ...
 async create(p: Product): Promise<Product> {
  ...
  async update(p: Product): Promise<void> {
    await this.drizzleService.db
      ...
      .set({ name: p.name, brand: p.brand, price: p.price, unit: p.unit, category: p.category, description: p.description, stock: p.stock, imageUrl: p.imageUrl, active: p.active })
      .where(eq(productsSchema.id, p.id!));
  }

  async delete(id: string): Promise<void> {
    await this.drizzleService.db.update(productsSchema).set({ active: false }).where(eq(productsSchema.id, id));
  }

  async findById(id: string): Promise<Product | null> {
    const result = await this.drizzleService.db.select().from(productsSchema).where(eq(productsSchema.id, id)).limit(1);
  ...
  async findAllPaginated(params: PaginationParams, search?: string): Promise<{ rows: Product[]; total: number }> {
    const { page, limit } = params;
    const offset = (page - 1) * limit;
    const whereClause = search
      ? or(ilike(productsSchema.name, `%${search}%`), ilike(productsSchema.category, `%${search}%`))
      : eq(productsSchema.active, true);

    const [rows, [countResult]] = await Promise.all([
      this.drizzleService.db.select().from(productsSchema).where(whereClause).limit(limit).offset(offset),
      this.drizzleService.db.select({ count: sql<number>`count(*)::int` }).from(productsSchema).where(whereClause),
    ]);

    return { rows: rows.map((r) => Product.restore(r)!), total: countResult.count };
  }
}
